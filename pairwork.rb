...
# require /Users/user/workspace/pairwork.rb'
...
class Cart
  attr_accessor :drinks,:name,:price,:number

  def initialize(name: nil, price: nil, number: nil)
    if (name.nil? || price.nil? || number.nil?)
      return false
    end
     @drinks =   self.drinks = Array.new(number) { |i| Drink.new(name: name, price: price) }
